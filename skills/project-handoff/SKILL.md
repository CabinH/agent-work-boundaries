...
With installed Hooks reviewed and trusted through `/hooks`: on confirmation transfer immediately; on rejection cancel; on any other prompt cancel the countdown before interpreting the prompt; after five minutes of silence let one durable local claimant make one automatic attempt per external phase. This is not external exactly-once: a possibly sent request blocks for inspection and is never retried automatically. Before an App Server connection, verify the running daemon matches the CLI and restart it only on a proven mismatch. After transfer, report the exact `resume_command`, direct the user to the created conversation, and prevent duplicate work in the old one; state that creation does not mean the UI focused it automatically.
